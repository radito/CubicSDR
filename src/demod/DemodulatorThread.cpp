  ...
  while (!stopping) {
    ...
    if (!squelched && (ati || modemDigital) && localAudioVisOutputQueue != nullptr && localAudioVisOutputQueue->empty()) {
      ...
      if (!localAudioVisOutputQueue->try_push(ati_vis)) {
        ...
        std::cout << "DemodulatorThread::run() cannot push ati_vis into "
                     "localAudioVisOutputQueue, is full !"
                  << std::endl;
...
bool DemodulatorThread::isDenoise() {
  return denoise;
}

void DemodulatorThread::setDenoise(bool denoise_in) {
  denoise = denoise_in;
}
