...
std::string& filterChars(std::string& s, const std::string& allowed) {
  s.erase(remove_if(s.begin(), s.end(), [&allowed](const char& c) { return allowed.find(c) == std::string::npos; }), s.end());
  return s;
}
...
class ActionDialogBookmarkCatastophe : public ActionDialog {
 public:
  ActionDialogBookmarkCatastophe() : ActionDialog(wxGetApp().getAppFrame(), wxID_ANY, wxT("Bookmark Last-Loaded Backup Failure :( :( :(")) {
    m_questionText->SetLabelText(
        wxT("All attempts to recover bookmarks have failed. \nWould you like to exit without touching any more save files?\nClick OK to exit without saving; "
            "or Cancel to continue anyways."));
  }
...
#ifdef _OSX_APP_
  cout << "Setting MacOSX assertion for no display sleep." << endl << flush;

  CFStringRef reasonForActivity = CFSTR("CubicSDR");
